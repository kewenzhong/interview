...
* 模板类中可以使用虚函数
* 一个类（无论是普通类还是类模板）的成员模板（本身是模板的成员函数）不能是虚函数。官方的解释是：当前的编译器都期望在处理类的定义的时候就能确定这个类的   虚函数表的大小，如果允许有类的虚成员模板函数，那么就必须要求编译器提前知道程序中所有对该类的该虚成员模板函数的调用，而这是不可行的。对于一个模板函     数，不同的模板参数会产生出不同的函数。这样的话，如果要知道类内包含多少个虚函数，就只能去代码中寻找
